...
// ==========================================================================
// 4. ONE-TAP NATIVE PHONE SYNC (NO FILE DOWNLOADS)
// ==========================================================================
function handleAutoCalendarSync() {
    const host = window.location.host;
    const isLocalhost = host.includes('localhost') || host.includes('127.0.0.1');

    if (isLocalhost) {
        alert("Live Calendar Auto-Sync requires deployment on Vercel so your phone can reach the public feed URL.");
        return;
    }
    ...
    if (isIOS) {
        // webcal:// tells iOS Safari to directly launch Apple Calendar's "Subscribe" prompt
        window.location.href = `webcal://${feedPath}`;
    } else {
        // Android / Desktop: Opens Google Calendar direct subscription link
        const gcalUrl = `https://calendar.google.com/calendar/r?cid=${encodeURIComponent(`https://${feedPath}`)}`;
